...
from patterns.distilled_local import ConsumerDeploymentConfig, _CONSUMER_CONFIGS  # noqa: PLC2701
...
_MODELS: list[ModelSpec] = [
    ...
    ModelSpec("DeepSeek-V3-671B", 671.0, num_layers=61, num_kv_heads=128, head_dim=128, is_moe=True),
]
...
def _build_rec_notes(
    ...
    notes: list[str] = []
    if model.is_moe:
        notes.append("MoE model: only active-expert weights load per token — effective VRAM usage lower.")
    if quant in {QuantizationType.GGUF_Q2_K, QuantizationType.GGUF_Q3_K_M}:
        notes.append("Aggressive quantization: noticeable quality degradation. Consider smaller model size.")
    if context_len > 32768:
        notes.append(f"KV cache at {context_len:,} tokens is large; reduce context_len to save VRAM.")
    if vram_gb <= 8:
...
def render_markdown_matrix(rows: list[MatrixRow], context_len: int) -> str:
    ...
    lines.append("| Model | Params | Quant | Weights (GB) | KV Cache (GB) | Total (GB) | PPL Delta |")
    lines.append("|-------|--------|-------|-------------|--------------|-----------|-----------|")
    prev_model = None
    for r in rows:
        sep = " " if r.model_name == prev_model else "**" + r.model_name.split("-")[-1] + "**"
